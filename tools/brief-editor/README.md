...
**Editor estático local para pulir briefs de onboarding antes de enviarlos al cliente.**

</div>

Acepta un brief en formato HTML (con JSON embebido) o JSON canónico suelto, permite corregir cualquier dato con un click sobre el documento y exporta el HTML actualizado. Sin servidor, sin LLM, sin auto-save. La edición vive en el navegador y el resultado es un `.html` standalone con todo el CSS inline, listo para enviar por email o publicar.

## Características

- **WYSIWYG por click**: cualquier dato del brief (cliente, citas, status, opciones, pendientes, narrativa, próxima reunión) abre un modal contextual al hacer click.
- **Sin servidor**: 100% client-side. Cargar, editar, exportar, cerrar la tab.
- **Roundtrip lossless**: el HTML exportado embebe el JSON canónico en un `<script id="brief-source">`. Permite recargarlo más tarde y seguir editando sin perder nada.
- **Validación con Zod**: el schema se valida al cargar el archivo con paths claros (`Campo inválido: blocks.0.fields.2.answer, expected string`).
...
> [!NOTE]
> Forma parte de [`brief-agent-templates`](../..). Consume el JSON que define [`agent/output-schema.md`](../../agent/output-schema.md) y renderiza con los tokens del design system de [`tools/render/`](../render/).
...
Abrir `http://localhost:5173/tools/brief-editor/` en el navegador y arrastrar un `.html` (con JSON embebido) o `.json` canónico a la dropzone.
...
El bundle queda en `dist/`. Configurado para servirse bajo `/tools/brief-editor/` (ver `vite.config.ts`).

## Flujo de uso

1. **Dropzone inicial**: arrastrar un `.html` (con JSON embebido) o `.json` canónico.
2. **Vista de edición**: el brief se renderiza a 920 px nativos, escalado al viewport.
...
> [!TIP]
> El editor avisa con un prompt nativo del navegador al recargar o cerrar la tab con cambios sin exportar.
